...
lint-licenses:
	$(call title, $@: $(GOOS))
	@cd $(MAKEFILE_DIR) \
		&& go-licenses check --include_tests --allowed_licenses=Apache-2.0,BSD-2-Clause,BSD-3-Clause,MIT,Unlicense \
		  --ignore gotest.tools \
		  ./...
	$(call footer, $@)
...
GOCMD := go
GOBUILD := CGO_ENABLED=0 $(GOCMD) build -trimpath -buildmode=pie -ldflags '$(LDFLAGS_RELEASE)'
GOBUILD_DEBUG := CGO_ENABLED=0 $(GOCMD) build -buildmode=pie -gcflags='$(GCFLAGS_DEBUG)' -ldflags '$(LDFLAGS_DEBUG)'

GOINSTALL := $(GOCMD) install

build: ## Build binary (PIE, release)
	@echo "Building $(NAME)..."
	@mkdir -p bin
	$(GOBUILD) -o $(BINARY_PATH) ./cmd/$(NAME)
	@echo "Binary built: $(BINARY_PATH)"

build-debug: ## Build binary (PIE, debug)
